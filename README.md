...
### v1.11.1 (2026-09-15)
- 🔥 **修复 Issue #3（音频丢失）**: 官方于 2026-09-14 把音频资源拆分到独立的 `catalog_sound.bundle`，主 catalog 中不再包含任何音频，导致新增剧情下载后没有语音、也看不到转码步骤。现支持多 catalog 合并加载
- ✨ **多 catalog 合并**: 主索引 + 音频索引分别解析后合并，附加索引获取失败只警告并跳过，不会让整个工具不可用
- ✨ **配置项**: 新增环境变量 `TSK_CATALOG_SOUND_BUNDLE_URL`（默认同目录下的 `catalog_sound.bundle`）
- 🐛 **控制台编码修复**: 输出被重定向时（如 `TSK_Updater.exe > log.txt`）不再因 GBK 编码无法输出 `•`、`✓` 而崩溃
- 📊 **日志改进**: 分别记录每个 catalog 的解析项数、新增项数与合并后的资源总数

### v1.11.0 (2026-08-14)
- 🐛 **音频格式查证**: 确认游戏音频真实格式为 m4a/aac（非 wav/ogg），临时文件改用真实扩展名，不再依赖 FFmpeg 嗅探
- 🐛 **健壮性修复**: 单个对象解码失败不再拖垮整个 bundle（原来一个坏音频会导致整包丢弃）
- 🐛 **错误可见**: bundle 重试失败、FFmpeg 转码失败现在都会写入 update_log.txt（原来静默丢弃）
- 🔒 **锁版本**: 锁死 UnityPy==1.25.0，防止未来版本不兼容
- ✨ **CDN 配置**: CDN 地址支持环境变量覆盖（TSK_CATALOG_BUNDLE_URL / TSK_RUNTIME_PATH），失效时明确提示
- 📊 **日志改进**: 完整记录 [1/5]~[5/5] 执行流程里程碑，便于排查
- 🧹 **代码重构**: get_target_relative_path 拆分为规则表+小函数（54.2万真实key验证100%一致）、全局变量收拢为 AppState、裸 except 改 logger 分级记录
